...
router.get(/hospital-lookup/, (req, res) => {
    ...
    if (postcodeLookup && regex.test(postcodeLookup)) {
        ...
        url = 'https://api.os.uk/search/places/v1/postcode?fn=CLASSIFICATION_CODE:C&maxresults=10&postcode=';
    
    } else if ( hospitalName && hospitalName.trim().length > 2 ) {

        queryString = hospitalName;
        url = 'https://api.os.uk/search/places/v1/find?fn=CLASSIFICATION_CODE:C&maxresults=10&query=';
